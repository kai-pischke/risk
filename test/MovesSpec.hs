...
cboth:: Contry -> Player -> Int -> GameState -> GameState
cboth c p n = (fromJust.(changetroop n).change c p)
    ...
    describe "Fortify" $ do
        ...
        context "Valid Inputs" $ do 
            ...
            it "correctly adds troops to one country from another for another player" $ do
                let t1g = (fromJust.(fortify Peru Brazil 1).nextPhase.nextPhase.nextTurne) game
    ...
    describe "Reinforce" $ do
        ...
        context "Valid Inputs" $ do 
            ...
            it "Reinforcing multiple country" $ do
                t1g = (fromJust.(reinforce [(WesternAustralia,1), (Siam, 3)]) testGame
